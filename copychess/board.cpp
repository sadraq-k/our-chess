...
void Board::ChangeScope(std::string& loc, std::string& where)
{
    ...
    try
    {
        if (square1.IsPiecesEmpty()) throw std::exception();
        else {
            if (!((int)square1.getColor() == Round % 2)) throw std::exception( );
            if (!square2.IsPiecesEmpty()) {
                if (square1.getColor() == square2.getColor()) throw std::exception();
