...
{	private:
	...
	char Destination_X_Coordinate[2] = {'a' , 'h'};
	int Destination_Y_Coordinate[2] = {1 , 1};*/
	...
	public:
		...
		bool checkMovement(){
			bool check = true; 
			if(Rook_Position->X != Rook_Destination->X  &&  Rook_Position->Y != Rook_Destination->Y ){
				check = false;
		...
		bool isPiecesBetween(SQUARECOORDINATE& RookPositon , SQUARECOORDINATE& RookDestination){
			bool check;
			if(RookPositon.X == RookDestination.X){
                ...
                {
					if(boardChess[i][RookPositon.Y] != emptysquarex )
                        check = false;
				} 
			}
                ...
                for (int  i = 0; i <= end; i++)
                {
                    if(boardChess[RookPositon.X][i] != emptysquarey )
                        check = false;
                }
		...
		void movement(int WhichRook = 1  ,/* char x = 'a' ,   int y = 1 */ HORIZONTAL x , VERTICAL y  ){
		bool check;
